...
class License(models.Model):
    ...
    license_code = models.CharField(
        max_length=256, unique=True, default=lambda: str(uuid.uuid4())
    )
    ...
    force = models.BooleanField(
        ...
        blank=True,
        help_text=_(
            "Override Envato license info (license type, status & amount), required for custom licenses!"
        ),
    )
    ...
    @classmethod
    def get_license(cls, license_code: str) -> License | None:
        try:
...
class App(models.Model):
    ...
    def verify_envato_license_code(self, code: str) -> tuple[dict[str, Any], bool]:
        data, valid = verify_envato_license_code(code, ENVATO_TOKEN)

        if valid:
            valid = False if self.envato_app_id != str(data["item"]["id"]) else valid

        return data, valid

    def is_valid_license(self, license: License, valid: bool = False) -> bool:
        if (
            license.app == self
            and (valid or license.force)
            and license.status == License.STATUS.ACTIVE
        ):
            return True
        return False

    def update_license(
        self, license: License, data: dict[str, Any], valid: bool = False
    ) -> None:
        if license.app == self:
            if not license.force:
                license_type = (
                    License.TYPES.REGULAR_LICENSE
                    if str(data.get("license")).lower() == "regular license"
                    else License.TYPES.EXTENDED_LICENSE
                )

                license.status = (
                    License.STATUS.ACTIVE if valid else License.STATUS.INACTIVE
                )
                license.license_type = (
                    license_type if valid else License.TYPES.REGULAR_LICENSE
                )
